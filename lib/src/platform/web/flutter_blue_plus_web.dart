...
final class FlutterBluePlusWeb extends FlutterBluePlusPlatform {
  ...
  final _devices = <DeviceIdentifier, wb.BluetoothDevice>{};

  // for instanceIds
  final _charCache = <DeviceIdentifier, Map<Guid, Map<Guid, List<wb.BluetoothRemoteGATTCharacteristic>>>>{};
  ...
  final _onDevicesChangedController = StreamController<List<wb.BluetoothDevice>>.broadcast();
  ...
  wb.BluetoothRemoteGATTCharacteristic _findCharacteristicOrThrow({
  ...
  int _instanceId(DeviceIdentifier devId, Guid serviceUuid, wb.BluetoothRemoteGATTCharacteristic target) {
  ...
  Future<bool> discoverServices(
    ...
    try {
      ...
      final devMap = _charCache.putIfAbsent(device.remoteId, () {
        return <Guid, Map<Guid, List<wb.BluetoothRemoteGATTCharacteristic>>>{};
      });
      ...
      for (final s in primaryServices) {
        ...
        final charsByUuid = devMap.putIfAbsent(Guid(s.uuid), () {
          return <Guid, List<wb.BluetoothRemoteGATTCharacteristic>>{};
        });
        ...
        charsByUuid
          ..clear()
          ..addAll(<Guid, List<wb.BluetoothRemoteGATTCharacteristic>>{});
        for (final c in chars) {
          (charsByUuid[Guid(c.uuid)] ??= <wb.BluetoothRemoteGATTCharacteristic>[]).add(c);
        }
  ...
  Future<bool> startScan(
    ...
    try {
      final filters = <wb.BluetoothLEScanFilterInit>[];

      for (final service in request.withServices) {
        filters.add(
          wb.BluetoothLEScanFilterInit(
            services: [
              service.str128.toJS,
            ].toJS,
          ),
      ...
      for (final manufacturerData in request.withMsd) {
        filters.add(
          wb.BluetoothLEScanFilterInit(
            manufacturerData: [
              wb.BluetoothManufacturerDataFilterInit(
      ...
      for (final serviceData in request.withServiceData) {
        filters.add(
          wb.BluetoothLEScanFilterInit(
            serviceData: [
              wb.BluetoothServiceDataFilterInit(
      ...
      final wb.RequestDeviceOptions options;

      if (filters.isNotEmpty) {
        options = wb.RequestDeviceOptions(
  ...
  void _handleCharacteristicValueChanged(
    ...
    final characteristic = event.target as wb.BluetoothRemoteGATTCharacteristic;
...
extension on wb.BluetoothDevice {
