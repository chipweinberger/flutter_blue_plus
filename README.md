...
But, due to interleaving writes, reads, and notifies all together in a single stream, it should only be used for small characteristics where each read, write, or notify always transfers the entire value. Otherwise, the various prtial data emitted would be unintelligible.
