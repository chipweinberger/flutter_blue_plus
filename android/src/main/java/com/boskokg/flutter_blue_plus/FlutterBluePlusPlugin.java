...
public class FlutterBluePlusPlugin
    implements FlutterPlugin, MethodCallHandler, RequestPermissionsResultListener, ActivityAware {
  ...
  public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
    ...
    switch (call.method) {
      ...
      case "startScan": {
        ensurePermissionBeforeAction(Build.VERSION.SDK_INT >= Build.VERSION_CODES.S ? Manifest.permission.BLUETOOTH_SCAN
            : Manifest.permission.ACCESS_FINE_LOCATION, (grantedScan, permissionScan) -> {
              if (grantedScan) {
                ensurePermissionBeforeAction(
                    ...
                    (grantedConnect, permissionConnect) -> {
                      if (grantedConnect)
                        ...
                        result.error(
                            "no_permissions",
                            String.format("flutter_blue plugin requires %s for scanning", permissionConnect), null);
                    });
              } else
                result.error(
                    "no_permissions", String.format("flutter_blue plugin requires %s for scanning", permissionScan),
                    null);
            });
        break;
      }
        ...
        ensurePermissionBeforeAction(
            ...
            (granted, permission) -> {
              if (!granted) {
                result.error(
                    "no_permissions",
                    String.format("flutter_blue plugin requires %s for obtaining connected devices", permission), null);
                return;
              }
      ...
      case "connect": {
        ensurePermissionBeforeAction(
            ...
            (granted, permission) -> {
              if (!granted) {
                result.error(
                    "no_permissions", String.format("flutter_blue plugin requires %s for new connection", permission),
                    null);
                return;
              }
      ...
      case "readCharacteristic": {
        ...
        try {
          ...
          characteristic = locateCharacteristic(gattServer, request.getServiceUuid(), request.getSecondaryServiceUuid(),
              request.getCharacteristicUuid());
        } catch (Exception e) {
      ...
      case "setNotification": {
        ...
        try {
          ...
          if (cccDescriptor == null) {
            // Some devices - including the widely used Bluno do not actually set the
            // CCCD_ID.
            // thus setNotifications works perfectly (tested on Bluno) without cccDescriptor
            log(LogLevel.INFO,
                "could not locate CCCD descriptor for characteristic: " + characteristic.getUuid().toString());
          }
        } catch (Exception e) {
        ...
        if (cccDescriptor != null) {
          if (!cccDescriptor.setValue(value)) {
            result.error("set_notification_error",
                "error when setting the descriptor value to: " + Arrays.toString(value), null);
            return;
          }
      ...
      case "requestMtu": {
        ...
        try {
          ...
          if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            ...
            result.error("requestMtu",
                "Only supported on devices >= API 21 (Lollipop). This device == " + Build.VERSION.SDK_INT, null);
          }
        } catch (Exception e) {
  ...
  private void startScan21(Protos.ScanSettings proto) throws IllegalStateException {
    ...
    if (offloadBatching && supportsOffloadBatching) {
      settings = new ScanSettings.Builder().setScanMode(scanMode)
          .setReportDelay((scanMode == 0) ? 2000 : (scanMode == 1) ? 1000 : (scanMode == 2) ? 500 : 500).build();
    } else {
  ...
  // BluetoothDeviceCache contains any other cached information not stored in
