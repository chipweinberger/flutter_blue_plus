...
public class FlutterBluePlusPlugin implements FlutterPlugin, MethodCallHandler, RequestPermissionsResultListener, ActivityAware {
  ...
  private static final HashMap<Integer,MethodChannel> channelsMap = new HashMap<Integer,MethodChannel>();
  ...
  private void setup(
    ...
    synchronized (initializationLock) {
      ...
      channelsMap.put(new Integer(instanceId),channel);
  ...
  private void tearDown() {
    synchronized (tearDownLock) {
      ...
      channel = null;
      channelsMap.remove(new Integer(instanceId));
  ...
  private void invokeMethodUIThread(final String name, final byte[] byteArray)
  {
    new Handler(Looper.getMainLooper()).post(() -> {
      synchronized (tearDownLock) {
        ...
        for (Map.Entry channelEntry : channelsMap.entrySet()){
          if (channelEntry.getValue() != null) {
            ((MethodChannel)channelEntry.getValue()).invokeMethod(name, byteArray);
          } else {
