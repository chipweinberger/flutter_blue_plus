#include "include/flutter_blue_plus/flutter_blue_plus.h"
...
void FlutterBluePlusPlugin::RegisterWithRegistrar(
  ...
  auto method =
      std::make_unique<flutter::MethodChannel<EncodableValue>>(
          registrar->messenger(), "flutter_blue_plus/method",
  ...
  auto event_paired_devices =
      std::make_unique<flutter::EventChannel<EncodableValue>>(
          registrar->messenger(), "flutter_blue_plus/event.pairedDevices",
          &flutter::StandardMethodCodec::GetInstance());        
...
void FlutterBluePlusPlugin::HandleMethodCall(
  ...
  if (method_name.compare("isBluetoothAvailable") == 0) {
    ...
    if (!deviceWatcher) {
      ...
      deviceWatcherAddedToken = deviceWatcher.Added({ this, &FlutterBluePlusPlugin::DeviceWatcher_Added });
      deviceWatcherUpdatedToken = deviceWatcher.Updated({ this, &FlutterBluePlusPlugin::DeviceWatcher_Updated });
      deviceWatcherRemovedToken = deviceWatcher.Removed({ this, &FlutterBluePlusPlugin::DeviceWatcher_Removed });
      deviceWatcherEnumerationCompletedToken = deviceWatcher.EnumerationCompleted({ this, &FlutterBluePlusPlugin::DeviceWatcher_EnumerationCompleted });
      deviceWatcherStoppedToken = deviceWatcher.Stopped({ this, &FlutterBluePlusPlugin::DeviceWatcher_Stopped });
    }
    deviceWatcher.Start();
...
bool FlutterBluePlusPlugin::IsDeviceWatcherStarted() {
...
bool FlutterBluePlusPlugin::IsDeviceWatcherRunning() {
    if (deviceWatcher == nullptr) return false;
...
void FlutterBluePlusPlugin::DeviceWatcher_Added(DeviceWatcher sender, DeviceInformation info) {
...
void FlutterBluePlusPlugin::DeviceWatcher_Updated(DeviceWatcher sender, DeviceInformationUpdate info) {
  // Watcher may have stopped while we were waiting for our chance to run.
...
void FlutterBluePlusPlugin::DeviceWatcher_Removed(DeviceWatcher sender, DeviceInformationUpdate info) {
  // Watcher may have stopped while we were waiting for our chance to run.
  auto value = info.Properties().TryLookup(L"System.Devices.Aep.IsConnected");
  auto isConnected = value && unbox_value<bool>(value);
...
void FlutterBluePlusPlugin::DeviceWatcher_EnumerationCompleted(DeviceWatcher sender, IInspectable obj) {
...
void FlutterBluePlusPlugin::DeviceWatcher_Stopped(DeviceWatcher sender, IInspectable obj) {
...
void FlutterBluePlusPlugin::CleanConnection(uint64_t bluetoothAddress, bool dispose) {
