    ...
    ActivityAware
{
   ...
   // ██   ██  ███████  ███████   ██████   ███████     ██
    ...
    private static int getLocalInstanceId(BluetoothGattCharacteristic target) {
    ...
    private List<BluetoothGattService> getMatchingServices(
        BluetoothGatt gatt,
        String primaryServiceUuid,
        String serviceUuid)
    {
        List<BluetoothGattService> matches = new ArrayList<>();
        boolean isSecondaryService = primaryServiceUuid != null;

        if (!isSecondaryService) {
            for (BluetoothGattService service : gatt.getServices()) {
                if (uuid128(service.getUuid()).equals(uuid128(serviceUuid))) {
                    matches.add(service);
                }
            }
            return matches;
        ...
        for (BluetoothGattService primary : gatt.getServices()) {
            if (!uuid128(primary.getUuid()).equals(uuid128(primaryServiceUuid))) {
                continue;
            }
            for (BluetoothGattService secondary : primary.getIncludedServices()) {
                if (uuid128(secondary.getUuid()).equals(uuid128(serviceUuid))) {
    ...
    private int getInstanceId(BluetoothGatt gatt, BluetoothGattCharacteristic target) {
        BluetoothGattService service = target.getService();
        if (service == null) return 0;

        BluetoothGattService primaryService = getPrimaryService(gatt, target);
        String primaryServiceUuid = primaryService != null ? uuidStr(primaryService.getUuid()) : null;
        String serviceUuid = uuidStr(service.getUuid());
        List<BluetoothGattService> services = getMatchingServices(gatt, primaryServiceUuid, serviceUuid);

        if (services.size() <= 1) {
            return getLocalInstanceId(target);
        }

        int idx = 0;
        for (BluetoothGattService candidateService : services) {
                                        ...
                                        Integer instanceId)
    {
        ...
        if(services.isEmpty()) {
            ...
            return new ChrFound(null, "primary service not found '" + serviceUuid + "'");
        }
        ...
        if (services.size() <= 1) {
            ...
            characteristic = getCharacteristicFromArray(characteristicUuid, service.getCharacteristics(), instanceId);
        } else {
            int idx = 0;
            for (BluetoothGattService service : services) {
                for (BluetoothGattCharacteristic candidate : service.getCharacteristics()) {
                    if (idx == instanceId && uuid128(candidate.getUuid()).equals(uuid128(characteristicUuid))) {
    ...
    private BluetoothGattCharacteristic getCharacteristicFromArray(String uuid, List<BluetoothGattCharacteristic> array, Integer instanceId)
    {
        for (BluetoothGattCharacteristic c : array) {
            if (uuid128(c.getUuid()).equals(uuid128(uuid))) {
                if (getLocalInstanceId(c) == instanceId) {
                    return c;
                }
            }
        }
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback()
    {
        ...
        // anyway. To handle this case, we make sure the connection is still expected, otherwise kill
        ...
        {
            BluetoothGatt connecting = mCurrentlyConnectingDevices.get(remoteId);
            BluetoothGatt connected = mConnectedDevices.get(remoteId);
            BluetoothGatt autoConnected = mAutoConnected.get(remoteId);

            // Match the GATT object itself so a callback from a previous connection attempt
            // cannot affect the current connection to the same device address.
            if (gatt == connecting || gatt == connected || gatt == autoConnected) {
                return false;
            }

            if (newState == BluetoothProfile.STATE_CONNECTED) {
                // This GATT object is no longer tracked, but Android reports it connected.
                // Disconnect it because we no longer expect this connection.
                log(LogLevel.DEBUG, "[unexpected connection] disconnecting now");
                gatt.disconnect();
            } else {
                // This GATT object is no longer tracked and is already disconnected.
                // No disconnect request is needed; just close it below.
                log(LogLevel.DEBUG, "[unexpected connection] disconnect complete");
            }

            // Only clear bonding state when no current GATT client can still need it.
            if (connecting == null && connected == null && autoConnected == null) {
                mBondingDevices.remove(remoteId);
                mBondingPins.remove(remoteId);
            }

            // Release only the obsolete client, without changing current connection state or
            // notifying Dart about a connection it no longer owns.
            gatt.close();
            return true;
        }
        ...
        public void onCharacteristicReceived(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, byte[] value, int status)
        {
            ...
            response.put("instance_id", getInstanceId(gatt, characteristic));
        ...
        public void onDescriptorRead(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status, byte[] value)
        {
            ...
            response.put("instance_id", getInstanceId(gatt, descriptor.getCharacteristic()));
            response.put("value", value);
            response.put("success", status == BluetoothGatt.GATT_SUCCESS ? 1 : 0);
            response.put("error_code", status);
            response.put("error_string", gattErrorString(status));
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status)
        {
            ...
            Integer instanceId = getInstanceId(gatt, descriptor.getCharacteristic());
    ...
    HashMap<String, Object> bmBluetoothCharacteristic(BluetoothDevice device, BluetoothGattCharacteristic characteristic, BluetoothGatt gatt) {
        ...
        map.put("instance_id", getInstanceId(gatt, characteristic));
    ...
    HashMap<String, Object> bmBluetoothDescriptor(BluetoothDevice device, BluetoothGattDescriptor descriptor, BluetoothGatt gatt) {
        ...
        map.put("instance_id", getInstanceId(gatt, descriptor.getCharacteristic()));
        return map;
    }
