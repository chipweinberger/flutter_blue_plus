...
- (NSString *)fullUUIDString {
  if (self.UUIDString.length == 4) {
    return [[NSString stringWithFormat:@"0000%@-0000-1000-8000-00805F9B34FB",
                                       self.UUIDString] lowercaseString];
  }
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  ...
  // STATE
  FlutterBluePlusStreamHandler *stateStreamHandler =
      [[FlutterBluePlusStreamHandler alloc] init];
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"state" isEqualToString:call.method]) {
    FlutterStandardTypedData *data = [self
        toFlutterData:[self toBluetoothStateProto:self->_centralManager.state]];
    result(data);
    ...
    ProtosScanSettings *request =
        [[ProtosScanSettings alloc] initWithData:[data data] error:nil];
    // UUID Service filter
    ...
    [self->_centralManager scanForPeripheralsWithServices:uuids
                                                  options:scanOpts];
    result(nil);
    ...
    // Cannot pass blank UUID list for security reasons. Assume all devices have
    // the Generic Access service 0x1800
    NSArray *periphs = [self->_centralManager
        retrieveConnectedPeripheralsWithServices:@[ [CBUUID
                                                     UUIDWithString:@"1800"] ]];
    ...
    FlutterStandardTypedData *data = [call arguments];
    ProtosConnectRequest *request =
        [[ProtosConnectRequest alloc] initWithData:[data data] error:nil];
    ...
    @try {
      CBPeripheral *peripheral = [self findPeripheral:remoteId];
      result([self toFlutterData:[self toDeviceStateProto:peripheral
    ...
    FlutterStandardTypedData *data = [call arguments];
    ProtosReadCharacteristicRequest *request =
        [[ProtosReadCharacteristicRequest alloc] initWithData:[data data]
                                                        error:nil];
    NSString *remoteId = [request remoteId];
    @try {
      // Find peripheral
      ...
      CBCharacteristic *characteristic =
          [self locateCharacteristic:[request characteristicUuid]
    ...
    ProtosReadDescriptorRequest *request =
        [[ProtosReadDescriptorRequest alloc] initWithData:[data data]
                                                    error:nil];
    NSString *remoteId = [request remoteId];
    @try {
      // Find peripheral
      ...
      CBCharacteristic *characteristic =
          [self locateCharacteristic:[request characteristicUuid]
                  ...
                  secondaryServiceId:[request secondaryServiceUuid]];
      // Find descriptor
      CBDescriptor *descriptor = [self locateDescriptor:[request descriptorUuid]
    ...
    ProtosWriteCharacteristicRequest *request =
        [[ProtosWriteCharacteristicRequest alloc] initWithData:[data data]
                                                         error:nil];
    NSString *remoteId = [request remoteId];
    @try {
      // Find peripheral
    ...
    ProtosWriteDescriptorRequest *request =
        [[ProtosWriteDescriptorRequest alloc] initWithData:[data data]
                                                     error:nil];
    NSString *remoteId = [request remoteId];
    @try {
      // Find peripheral
    ...
    FlutterStandardTypedData *data = [call arguments];
    ProtosSetNotificationRequest *request =
        [[ProtosSetNotificationRequest alloc] initWithData:[data data]
                                                     error:nil];
    NSString *remoteId = [request remoteId];
    @try {
      // Find peripheral
      ...
      [peripheral setNotifyValue:[request enable]
               forCharacteristic:characteristic];
      result(nil);
    } @catch (FlutterError *e) {
      result(e);
    ...
    @try {
      ...
      result([self toFlutterData:[self toMtuSizeResponseProto:peripheral
                                                          mtu:mtu]]);
    } @catch (FlutterError *e) {
      result(e);
    }
...
- (CBPeripheral *)findPeripheral:(NSString *)remoteId {
  NSArray<CBPeripheral *> *peripherals = [_centralManager
      retrievePeripheralsWithIdentifiers:@[ [[NSUUID alloc]
                                             initWithUUIDString:remoteId] ]];
...
- (CBCharacteristic *)locateCharacteristic:(NSString *)characteristicId
  ...
  CBService *primaryService = [self getServiceFromArray:serviceId
                                                  array:[peripheral services]];
  if (primaryService == nil || [primaryService isPrimary] == false) {
    @throw [FlutterError
        errorWithCode:@"locateCharacteristic"
  ...
  if (secondaryServiceId.length) {
    secondaryService =
        [self getServiceFromArray:secondaryServiceId
...
- (CBDescriptor *)locateDescriptor:(NSString *)descriptorId
  ...
  if (descriptor == nil) {
    @throw [FlutterError
        errorWithCode:@"locateDescriptor"
...
- (CBService *)findPrimaryService:(CBService *)secondaryService
                       peripheral:(CBPeripheral *)peripheral {
  for (CBService *s in [peripheral services]) {
    for (CBService *ss in [s includedServices]) {
      if ([[ss.UUID UUIDString]
              isEqualToString:[secondaryService.UUID UUIDString]]) {
        return s;
      }
    }
  }
...
- (CBService *)getServiceFromArray:(NSString *)uuidString
...
- (CBCharacteristic *)getCharacteristicFromArray:(NSString *)uuidString
...
- (CBDescriptor *)getDescriptorFromArray:(NSString *)uuidString
...
- (void)centralManager:(CBCentralManager *)central
  ...
  ProtosScanResult *result = [self toScanResultProto:peripheral
...
- (void)centralManager:(CBCentralManager *)central
  ...
  uint32_t mtu = [self getMtu:peripheral];
  [_channel
      invokeMethod:@"MtuSize"
         arguments:[self toFlutterData:[self toMtuSizeResponseProto:peripheral
                                                                mtu:mtu]]];

  // Send connection state
  [_channel
      invokeMethod:@"DeviceState"
         arguments:[self
                       toFlutterData:[self
                                         toDeviceStateProto:peripheral
                                                      state:peripheral.state]]];
}
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  for (CBService *s in [peripheral services]) {
    ...
    // [peripheral discoverIncludedServices:nil forService:s]; // Secondary
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  [_characteristicsThatNeedDiscovered
      addObjectsFromArray:service.characteristics];
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  if (_servicesThatNeedDiscovered.count > 0 ||
      _characteristicsThatNeedDiscovered.count > 0) {
    // Still discovering
    return;
  }
  // Send updated tree
  ProtosDiscoverServicesResult *result =
      [self toServicesResultProto:peripheral];
  [_channel invokeMethod:@"DiscoverServicesResult"
               arguments:[self toFlutterData:result]];
}
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  ProtosReadCharacteristicResponse *result =
      [[ProtosReadCharacteristicResponse alloc] init];
  [result setRemoteId:[peripheral.identifier UUIDString]];
  [result setCharacteristic:[self toCharacteristicProto:peripheral
                                         characteristic:characteristic]];
  [_channel invokeMethod:@"ReadCharacteristicResponse"
               arguments:[self toFlutterData:result]];

  // on iOS, this method also handles notification values
  ProtosOnCharacteristicChanged *onChangedResult =
      [[ProtosOnCharacteristicChanged alloc] init];
  [onChangedResult setRemoteId:[peripheral.identifier UUIDString]];
  [onChangedResult
      setCharacteristic:[self toCharacteristicProto:peripheral
                                     characteristic:characteristic]];
  [_channel invokeMethod:@"OnCharacteristicChanged"
               arguments:[self toFlutterData:onChangedResult]];
}
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  ProtosWriteCharacteristicRequest *request =
      [[ProtosWriteCharacteristicRequest alloc] init];
  ...
  ProtosWriteCharacteristicResponse *result =
      [[ProtosWriteCharacteristicResponse alloc] init];
  [result setRequest:request];
  [result setSuccess:(error == nil)];
  [_channel invokeMethod:@"WriteCharacteristicResponse"
               arguments:[self toFlutterData:result]];
}
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  if (cccd == nil || error != nil) {
    // Send error
    ProtosSetNotificationResponse *response =
        [[ProtosSetNotificationResponse alloc] init];
    [response setRemoteId:[peripheral.identifier UUIDString]];
    [response setCharacteristic:[self toCharacteristicProto:peripheral
                                             characteristic:characteristic]];
    [response setSuccess:false];
    [_channel invokeMethod:@"SetNotificationResponse"
                 arguments:[self toFlutterData:response]];
    return;
  }
...
- (void)peripheral:(CBPeripheral *)peripheral
  ...
  if ([descriptor.characteristic.service isPrimary]) {
    ...
    [q setSecondaryServiceUuid:[descriptor.characteristic.service
                                       .UUID fullUUIDString]];
    CBService *primaryService = [self
        findPrimaryService:[descriptor.characteristic service]
                peripheral:[descriptor.characteristic.service peripheral]];
    [q setServiceUuid:[primaryService.UUID fullUUIDString]];
  }
  ProtosReadDescriptorResponse *result =
      [[ProtosReadDescriptorResponse alloc] init];
  [result setRequest:q];
  int value = [descriptor.value intValue];
  [result setValue:[NSData dataWithBytes:&value length:sizeof(value)]];
  [_channel invokeMethod:@"ReadDescriptorResponse"
               arguments:[self toFlutterData:result]];

  // If descriptor is CCCD, send a SetNotificationResponse in case anything is
  // awaiting
  if ([descriptor.UUID.UUIDString isEqualToString:@"2902"]) {
    ...
    [response setRemoteId:[peripheral.identifier UUIDString]];
    [response
        setCharacteristic:[self
                              toCharacteristicProto:peripheral
...
- (void)peripheral:(CBPeripheral *)peripheral
    didWriteValueForDescriptor:(CBDescriptor *)descriptor
                         error:(NSError *)error {
  ProtosWriteDescriptorRequest *request =
      [[ProtosWriteDescriptorRequest alloc] init];
  [request setRemoteId:[peripheral.identifier UUIDString]];
  [request
      setCharacteristicUuid:[descriptor.characteristic.UUID fullUUIDString]];
  [request setDescriptorUuid:[descriptor.UUID fullUUIDString]];
  if ([descriptor.characteristic.service isPrimary]) {
    [request
        setServiceUuid:[descriptor.characteristic.service.UUID fullUUIDString]];
  } else {
    [request setSecondaryServiceUuid:[descriptor.characteristic.service
                                             .UUID fullUUIDString]];
    CBService *primaryService = [self
        findPrimaryService:[descriptor.characteristic service]
                peripheral:[descriptor.characteristic.service peripheral]];
  ...
  ProtosWriteDescriptorResponse *result =
      [[ProtosWriteDescriptorResponse alloc] init];
  [result setRequest:request];
  [result setSuccess:(error == nil)];
  [_channel invokeMethod:@"WriteDescriptorResponse"
               arguments:[self toFlutterData:result]];
}
...
- (FlutterStandardTypedData *)toFlutterData:(GPBMessage *)proto {
  FlutterStandardTypedData *data =
      [FlutterStandardTypedData typedDataWithBytes:[[proto data] copy]];
  return data;
}
...
- (ProtosScanResult *)toScanResultProto:(CBPeripheral *)peripheral
  ...
  [ads setConnectable:[advertisementData[CBAdvertisementDataIsConnectable]
                          boolValue]];
  ...
  if (manufData.length > 2) {
    ...
    [[ads manufacturerData]
        setObject:[manufData
                      subdataWithRange:NSMakeRange(2, manufData.length - 2)]
...
- (ProtosBluetoothDevice *)toDeviceProto:(CBPeripheral *)peripheral {
  ...
  [result setRemoteId:[[peripheral identifier] UUIDString]];
  [result
      setType:ProtosBluetoothDevice_Type_Le]; // TODO: Does iOS differentiate?
  return result;
}

- (ProtosDeviceStateResponse *)toDeviceStateProto:(CBPeripheral *)peripheral
                                            state:(CBPeripheralState)state {
  ProtosDeviceStateResponse *result = [[ProtosDeviceStateResponse alloc] init];
  switch (state) {
  case CBPeripheralStateDisconnected:
    [result
        setState:ProtosDeviceStateResponse_BluetoothDeviceState_Disconnected];
    break;
...
- (ProtosDiscoverServicesResult *)toServicesResultProto:
    (CBPeripheral *)peripheral {
  ProtosDiscoverServicesResult *result =
      [[ProtosDiscoverServicesResult alloc] init];
  [result setRemoteId:[peripheral.identifier UUIDString]];
...
- (ProtosConnectedDevicesResponse *)toConnectedDeviceResponseProto:
    (NSArray<CBPeripheral *> *)periphs {
  ProtosConnectedDevicesResponse *result =
      [[ProtosConnectedDevicesResponse alloc] init];
...
- (ProtosBluetoothService *)toServiceProto:(CBPeripheral *)peripheral
  ...
  for (CBCharacteristic *c in [service characteristics]) {
    [characteristicProtos addObject:[self toCharacteristicProto:peripheral
                                                 characteristic:c]];
  }
  ...
  for (CBService *s in [service includedServices]) {
    [includedServicesProtos addObject:[self toServiceProto:peripheral
                                                   service:s]];
  }
...
- (ProtosBluetoothCharacteristic *)
    toCharacteristicProto:(CBPeripheral *)peripheral
           characteristic:(CBCharacteristic *)characteristic {
  ProtosBluetoothCharacteristic *result =
      [[ProtosBluetoothCharacteristic alloc] init];
  [result setUuid:[characteristic.UUID fullUUIDString]];
  [result setRemoteId:[peripheral.identifier UUIDString]];
  [result
      setProperties:[self
                        toCharacteristicPropsProto:characteristic.properties]];
  [result setValue:[characteristic value]];
  NSLog(@"uuid: %@ value: %@", [characteristic.UUID fullUUIDString],
        [characteristic value]);
  NSMutableArray *descriptorProtos = [NSMutableArray new];
  for (CBDescriptor *d in [characteristic descriptors]) {
    [descriptorProtos addObject:[self toDescriptorProto:peripheral
                                             descriptor:d]];
  }
  ...
  if ([characteristic.service isPrimary]) {
    ...
    [result
        setSecondaryServiceUuid:[characteristic.service.UUID fullUUIDString]];
    CBService *primaryService =
        [self findPrimaryService:[characteristic service]
...
- (ProtosBluetoothDescriptor *)toDescriptorProto:(CBPeripheral *)peripheral
...
- (ProtosCharacteristicProperties *)toCharacteristicPropsProto:
    (CBCharacteristicProperties)props {
  ProtosCharacteristicProperties *result =
      [[ProtosCharacteristicProperties alloc] init];
  [result setBroadcast:(props & CBCharacteristicPropertyBroadcast) != 0];
  [result setRead:(props & CBCharacteristicPropertyRead) != 0];
  [result
      setWriteWithoutResponse:(props &
  ...
  [result setAuthenticatedSignedWrites:
              (props & CBCharacteristicPropertyAuthenticatedSignedWrites) != 0];
  [result
      setExtendedProperties:(props &
                             CBCharacteristicPropertyExtendedProperties) != 0];
  [result setNotifyEncryptionRequired:
              (props & CBCharacteristicPropertyNotifyEncryptionRequired) != 0];
  [result
      setIndicateEncryptionRequired:
          (props & CBCharacteristicPropertyIndicateEncryptionRequired) != 0];
  return result;
}

- (ProtosMtuSizeResponse *)toMtuSizeResponseProto:(CBPeripheral *)peripheral
                                              mtu:(uint32_t)mtu {
